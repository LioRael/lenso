...
by the owner-local generator and `cargo test --locked -p lenso-api-contracts --test generated_artifacts`.
Provider v1 and System v1 keep
...
and checked by the owner-local generator and `cargo test --locked -p lenso-api-contracts --test architecture`.
Repository scanning
...
The committed HTTP and gRPC binding fixtures are regenerated by
`cargo run --locked -p lenso-api-contracts --bin generate-contracts` and checked by the
`lenso-api-contracts` `generated_artifacts` integration test.
...
The owner-local generator publishes the generic envelope schema plus the support Event
Contract artifact and round-trip fixture under `contracts/events/`. Their
freshness is enforced by the `lenso-api-contracts` `generated_artifacts` integration test.
