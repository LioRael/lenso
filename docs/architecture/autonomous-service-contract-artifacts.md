...
Generated HTTP and gRPC
clients resolve logical Service References directly, enforce the same Service-local circuit and
bulkhead state, preserve their native protocol failures, and never use the Provider proxy path.
Unknown write safety remains non-retryable. Explicit fallback declarations name a handler, but
the owning Module or Service composition must register that handler; platform code never creates
a business result when one is absent.
