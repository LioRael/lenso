...
`requires_key`. The generated server binding rejects missing or expired absolute deadlines and
required Idempotency Keys before the business handler. The generated client resolves a logical
Service Reference directly, preserves response status, headers, body, and standard problem JSON,
and never uses the Provider proxy path.

The initial Call Policy permits at most one automatic retry, only for 429, 502, 503, or 504 and
only when the operation is declared idempotent or carries a required key. Unknown safety,
non-retryable failures, and the attempt limit expose stable no-retry reasons. The committed
`contracts/services/support-http.v1.bindings.json` fixture is regenerated by `just generate` and
checked by `just generated-check` and `just arch-check`.
