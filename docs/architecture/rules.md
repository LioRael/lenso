...
- Autonomous Services declared with `lenso.service.v2` use the separate `lenso-autonomous-service` runtime boundary. Keep their Store, health, migration, shutdown, and local Story Segment state Service-owned; do not route this profile through Host or Provider startup. Business routes and migrations must still come from Modules rather than platform-owned business handlers.
