...
- Console frontend contributions must use `ModuleManifest.console` plus the digest-bound `ConsoleUiArtifact` in the same Module Release. Isolated UI may use only `lenso.console-bridge.v1` and exact composition grants.
