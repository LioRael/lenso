...
- Managed-Service System Plane routes must use `platform-system-plane` admission: authenticated transport, audience-bound Workload Identity, and the current Service-owned Enrollment Grant are all required. Production enrollment state lives in the Service Store and enforces one active Console Service Principal, expiry, revocation, and monotonic authorization epochs. Capability Providers own their observations and operations, advertise the exact contract/schema digest through Core, remain independent of Console state and UI vocabulary, and keep read-only snapshots out of Story mutation.
