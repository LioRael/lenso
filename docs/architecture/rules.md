...
The checker fails on forbidden module folders, forbidden cross-module imports inside module source code, stale generated contracts, missing OpenAPI artifacts, malformed contract JSON/YAML, missing event contracts referenced by source code, event contract name/path mismatches, missing runtime function contracts for registered module runtime functions, and runtime function contract name/path mismatches. Console source guardrails live in the sibling frontend repository.
