...
- Extraction readiness rules and the versioned report contract belong in the public `lenso-service` surface. CLI-owned analyzers may collect repository evidence, but they must call the shared evaluator and render its report; they must not duplicate classification rules. Readiness analysis is read-only and must never write repository files, start Workloads, move data, or change authority. Provider v1 and System v1 semantics remain unchanged.
