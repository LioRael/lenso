...
- Module manifest lint rules belong in `platform-module` and are exposed through backend metadata endpoints such as `/admin/data/modules`. Runtime Console screens may filter, group, and render lint results, but must not duplicate backend manifest lint rules in TypeScript.
- Remote modules may provide manifests, declared HTTP route metadata, and schema-admin read data through `platform-module-remote`. Declared remote HTTP routes are metadata only; they must not be mounted or proxied until the host policy in `docs/architecture/module-remote-http-proxy.md` is implemented. Remote runtime functions and event handlers also require their own specs before implementation.
