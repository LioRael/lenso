...
- Module install, update, uninstall, restore, approval, crash-resume, and repair semantics belong in `lenso-module-management`. CLI and Runtime Console adapters must call this shared engine; they must not duplicate lifecycle rules or shell out to one another.
...
- The trusted catalog adapter materializes `.lenso/module-planning-context.json`; Runtime Console and other clients submit only a `ModuleRootChange` to `/admin/modules/plans/preview`. The backend must read target-owned desired/lock state and call the shared Plan Builder. HTTP adapters must not accept caller-authored resolved locks, Cargo files, effect lists, or shell commands.
...
- `/admin/modules/operations/*` is the Runtime Console execution surface. Exact plans, operation state, approvals, effect receipts, backups, fencing leases, and hash-chained journals live under `.lenso/module-management`; callers provide revisions and business decisions, never state transitions or receipts.
...
- Crash resume evidence is observed and constructed by the backend from the bound plan, journaled receipts, and current target bytes. Runtime Console may request resume with the expected revision but must not author completed-effect or idempotence evidence.
