...
Configured remote modules are loaded at startup through `platform-module-remote`. The current Remote slices support manifest loading, declared HTTP route metadata, schema-admin reads, admin surface metadata, and host-owned HTTP proxying for declared GET, POST, PUT, PATCH, and DELETE routes. Third-party module packaging and ecosystem boundaries are specified in `docs/architecture/third-party-modules.md`. Route proxying is specified separately in `docs/architecture/module-remote-http-proxy.md`. Remote runtime execution is scoped in `docs/architecture/module-remote-runtime.md`. Event handling and marketplace trust are separate future specs.
