...
The module registry endpoint under `/admin/data/modules` is also the source of truth for module manifest health. `platform-admin-data` derives manifest lint results from `platform-module` helpers, including HTTP route declaration checks, and returns those lint results with the module metadata. The Runtime Console renders these `route_lints` as Manifest Lints; it must not reimplement the lint rules locally.
