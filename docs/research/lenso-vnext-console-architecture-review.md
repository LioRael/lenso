...
1. The current Console is already an independently deployed Lenso application,
   ...
   Story Modules ([composition.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/composition.rs:68)).
...
3. The current independent identity boundary is sound: Console operators and
   ...
   as a vNext seam ([console_access.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/console_access.rs:689),
   [console_access.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/console_access.rs:1607)).
...
6. The current UI loader does **not** execute arbitrary remote URLs. An
   ...
   the Console origin ([console_artifacts.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/console_artifacts.rs:148),
   [console_artifacts.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/console_artifacts.rs:448),
   [console-module-runtime.ts](/Users/leosouthey/Projects/framework/lenso-console/src/app/console-module-runtime.ts:288)).
...
| --- | --- | --- | --- |
| Process and release boundary | A dedicated service boots API, migration, Worker, and combined API/Worker workloads ([api.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/bin/api.rs:1), [serve.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/bin/serve.rs:1), [worker.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/bin/worker.rs:1), [migrate.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/bin/migrate.rs:1)). | Yes, if Console is a durable multi-App operator product; no for an embedded developer view. | Deployment is an App choice, not a Kernel or Module kind. |
| Operator authentication and sessions | The fixed composition embeds Auth and Password Auth ([composition.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/composition.rs:109)). Console Access creates users and sessions through concrete Auth repositories and lists/revokes persisted sessions ([console_access.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/console_access.rs:772), [console_access.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/console_access.rs:926)). | The **separate operator trust domain** is essential. Owning a specific Auth implementation or local user database is not. | Console should require authentication/session Capabilities. Local password, OIDC, or an external identity provider can satisfy them. |
| Authorization and organizations | Console Access owns administrator authority, organizations, and per-managed-Service grants; the schema persists administrators, grants, and audit rows ([0001_create_console_access.sql](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/console_access/migrations/0001_create_console_access.sql:1)). Organization data is a separate schema but has a direct foreign key to Auth users ([0001_create_organization_schema.sql](/Users/leosouthey/Projects/framework/lenso-organization-module/crates/organization/migrations/0001_create_organization_schema.sql:33)). | Console-owned operator-to-target authorization is essential for a central Console. Organization is one policy implementation, not universally mandatory. | `console.access` should depend on typed identity/policy Capabilities, not concrete repositories or shared tables. |
| PostgreSQL state | The production deployment requires a dedicated PostgreSQL database and volume plus a separate migration workload ([compose.yml](/Users/leosouthey/Projects/framework/lenso-console/service/compose.yml:21)). Readiness directly checks `ctx.db` ([console_shell.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/console_shell.rs:272)). | Durable state is needed only for installed stateful Modules. PostgreSQL itself is not part of Console semantics. | Each stateful Module owns its state contract and migration command. An embedded read-only Console can be stateless. |
| Outbox, functions, and Workflow runtime | `serve` always starts the generic Lenso Worker in normal mode. That Worker polls the platform Outbox, scheduler, and function runtime ([boot.rs](/Users/leosouthey/Projects/framework/lenso/crates/lenso/src/host/boot.rs:94), [lenso-worker/src/lib.rs](/Users/leosouthey/Projects/framework/lenso/crates/lenso-worker/src/lib.rs:80)). Yet the Console Access, Registry, and Story manifests declare HTTP/UI concerns rather than Console-owned workflow definitions ([console_access.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/console_access.rs:53), [system_registry/mod.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/system_registry/mod.rs:156), [story/module.rs](/Users/leosouthey/Projects/framework/lenso-console/service/modules/story/src/module.rs:80)). | Not intrinsically. Asynchronous collection, reliable audit export, or long-running operations may opt into these facilities. | Install Scheduler, Durable Event, Workflow, or Outbox Modules only when another Console Module requires their semantics. |
| Target registry and connection state | System Registry owns managed-Service enrollment, target endpoints, signed evidence, Core documents, topology, workload operations, and audit state ([0001_create_managed_service_registry.sql](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/system_registry/migrations/0001_create_managed_service_registry.sql:3), [0002_create_system_connections.sql](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/system_registry/migrations/0002_create_system_connections.sql:1)). | A central multi-App Console needs a target catalog and trust relationship. A local embedded Console does not. | Separate target catalog, trust/enrollment, invocation, diagnostics, and control Capabilities; do not preserve one mandatory System Registry aggregate by default. |
| Runtime and business-operation gateway | The browser receives a receipt-bound client. The server then revalidates topology, release identity, contracts, operator grants, and target identity before forwarding a request ([console-module-client.ts](/Users/leosouthey/Projects/framework/lenso-console/src/app/console-module-client.ts:35), [surface_gateway.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/surface_gateway.rs:195)). | A mediation boundary is essential when Console and target App are independent. Its current release/digest/System-topology machinery is not. | Bind generated clients to portable Capability contracts and ActorAssertion audience. Keep read, mutation, diagnostics, and workload-control authorities distinct. |
| Story aggregation | Story is marked optional in the Console composition ([composition.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/composition.rs:98)). Its local query joins platform Outbox, function runs, story events, and provider calls ([queries.rs](/Users/leosouthey/Projects/framework/lenso-console/service/modules/story/src/backend/queries.rs:4)); its federation layer explicitly says it is read-only and cannot advance workflows or write a source Service Store ([federation.rs](/Users/leosouthey/Projects/framework/lenso-console/service/modules/story/src/federation.rs:1)). | No. It is a separately installable observability product. | `story.query` and `story.collect` should be optional Modules consuming business/audit evidence and/or Runtime Diagnostics. OTel-only Apps need neither. |
| UI shell and contributions | The Shell serves the SPA and content-addressed artifacts; dynamic routes select a declared surface, validate target context, load the ESM, and inject a restricted client ([console_shell.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/console_shell.rs:91), [dynamic-console-module.tsx](/Users/leosouthey/Projects/framework/lenso-console/src/app/dynamic-console-module.tsx:78), [dynamic-console-module.tsx](/Users/leosouthey/Projects/framework/lenso-console/src/app/dynamic-console-module.tsx:342)). | A Console needs a Shell and a contribution model. Artifact storage and execution trust policy can vary. | UI Contribution discovery, asset resolution, UI execution, and capability-client injection should be separate seams. |
| Workload control and recovery authority | System Registry stores workload operations; deployment recovery also fences normal and restore workloads and disables background work ([0003_create_workload_control_operations.sql](/Users/leosouthey/Projects/framework/lenso-console/service/src/modules/system_registry/migrations/0003_create_workload_control_operations.sql:1), [lib.rs](/Users/leosouthey/Projects/framework/lenso-console/service/src/lib.rs:55)). | Not part of the local-first Console minimum. It becomes relevant to deployment operations and later distributed systems. | Keep it out of the first Console/App connector and out of Kernel. Add an explicitly authorized control Module later. |
...
([migrations.rs](/Users/leosouthey/Projects/framework/lenso/crates/platform-core/src/migrations.rs:11)).
...
and runtime control ([platform-system-plane/src/lib.rs](/Users/leosouthey/Projects/framework/lenso/crates/platform-system-plane/src/lib.rs:31),
[lenso-api/src/lib.rs](/Users/leosouthey/Projects/framework/lenso/crates/lenso-api/src/lib.rs:146)).
...
([CONTEXT.md](/Users/leosouthey/Projects/framework/lenso-console/CONTEXT.md:59)).
