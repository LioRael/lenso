...
- Agent-ready behavior is a cross-cutting command and contract requirement, not a separate code-generation feature.
- Skills can compose reliable workflows from the same machine-readable plans and evidence humans inspect.
- Production authority is not implied by an agent's ability to generate a valid plan.
- Audit evidence identifies the plan, actor, approvals, applied steps, and resulting System state.
- Human and agent workflows use the same Policy Packs and verification commands.
