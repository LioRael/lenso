...
for variable in GITHUB_TOKEN GITHUB_REPOSITORY GITHUB_REF GITHUB_EVENT_NAME RELEASE_SHA RELEASE_SET RELEASE_MODE EXPECTED_REPOSITORY; do
  require_env "$variable"
done

[[ "$GITHUB_REPOSITORY" == "$EXPECTED_REPOSITORY" ]] ||
  fail "release workflow is restricted to $EXPECTED_REPOSITORY"
...
while IFS=$'\t' read -r package version; do
  registry_status="$(
    curl --silent --show-error --location --retry 2 \
      --user-agent "Lenso-release-gate/1.0 (https://github.com/${EXPECTED_REPOSITORY})" \
...
done < <(jq -r --argjson packages "$metadata" '$packages.packages[] | select(.publish == null or ((.publish | type) == "array" and (.publish | index("crates-io") != null))) | [.name, .version] | @tsv' <<<"null")
