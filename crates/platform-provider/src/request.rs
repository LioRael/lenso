        ...
        ProxyRequestBody::Json(body) => {
            ...
            Some(serde_json::from_slice(&body).map_err(|error| {
                AppError::new(
                    ...
                    format!("provider HTTP proxy request body was invalid JSON: {error}"),
                )
            })?)
        }
    };
...
fn validate_json_body_policy<'a>(
    ...
    if !method_allows_request_body(method) {
        return Err(AppError::new(
            ErrorCode::Validation,
            format!(
                "provider HTTP proxy method {} does not accept a request body",
    ...
    if body.len() > MAX_PROXY_REQUEST_BYTES {
        return Err(AppError::new(
            ErrorCode::Validation,
            format!(
                "provider HTTP proxy request body exceeded {MAX_PROXY_REQUEST_BYTES} bytes: {} bytes",
                body.len()
            ),
        ));
    }
...
fn validated_json_content_type(headers: &HeaderMap) -> AppResult<&str> {
    let Some(content_type) = headers
        ...
        return Err(AppError::new(
            ErrorCode::Validation,
            "provider HTTP proxy request content-type was missing",
        ));
    };
    ...
    Err(AppError::new(
        ErrorCode::Validation,
        format!("provider HTTP proxy request content-type was not JSON: {content_type}"),
    ))
}
...
mod tests {
    ...
    fn forwards_only_allowed_get_headers_and_host_auth() {
        ...
        let request = apply_proxy_request_policy(
            client.get("http://provider.test/contacts"),
    ...
    fn accepts_json_body_for_future_body_methods() {
        ...
        let client = reqwest::Client::new();
        let request = apply_proxy_request_policy(
            client.post("http://provider.test/contacts"),
