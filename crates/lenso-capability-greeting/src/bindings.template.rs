...
use lenso_kernel::{NativeApp, NativeRequestEndpoint, RequestCapability, RuntimeFailure};
...
#[derive(Debug)]
pub struct __CAPABILITY__Client<'a> { app: &'a NativeApp, caller: &'a str }
impl<'a> __CAPABILITY__Client<'a> {
    pub fn new(app: &'a NativeApp, caller: &'a str) -> Result<Self, RuntimeFailure> {
        app.ensure_binding::<__CAPABILITY__>(caller)?;
        Ok(Self { app, caller })
    }
    pub async fn __OPERATION_FN__(&self, request: __OPERATION_TYPE__Request) -> Result<__OPERATION_TYPE__Response, __CAPABILITY__InvocationError> {
        self.app.invoke::<__CAPABILITY__>(self.caller, GREET_OPERATION, request).await
