...
fn main() {
    ...
    for &graph_size in &configuration.sizes {
        ...
        let stages = [
            measure_stage("resolved_plan_validation", &configuration, || {
                let result = black_box(&plan).validate();
                black_box(result).expect("the benchmark Plan must remain valid");
            }),
            measure_stage("activation_order_derivation", &configuration, || {
                let order = black_box(&plan)
                    .activation_order()
                    .expect("the benchmark Plan must remain acyclic");
                assert_eq!(black_box(order).len(), graph_size);
            }),
            measure_stage("execution_adapter_preparation", &configuration, || {
                let prepared = BenchmarkAdapter
                    .prepare(black_box(&plan))
            ...
            measure_pre_timed_stage("complete_kernel_start", &configuration, || {
                ...
                let measured_plan = black_box(plan.clone());
    ...
    let output = json!({
        "schema": "lenso.startup-attribution.v1",
