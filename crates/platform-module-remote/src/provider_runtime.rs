use crate::{
    RemoteHttpProxyRegistry, RemoteModuleConfig, RemoteModuleSource, RemoteModuleTransport,
};
...
pub struct LoadedProviderRuntime {
    modules: Vec<Module>,
    configs: Vec<RemoteModuleConfig>,
}
    ...
    pub async fn load_verified(self) -> AppResult<LoadedProviderRuntime> {
        ...
        for provider in &self.plan.providers {
            ...
            for locked in &provider.modules {
                let config = module_config(locked, endpoint, bearer.as_deref());
...
impl LoadedProviderRuntime {
    #[must_use]
    pub fn proxy_registry(&self) -> RemoteHttpProxyRegistry {
        RemoteHttpProxyRegistry::from_modules(&self.modules, &self.configs)
    }
    ...
    #[must_use]
    pub fn into_parts(self) -> (Vec<Module>, Vec<RemoteModuleConfig>) {
...
fn module_config(
    ...
    let transport = match endpoint.binding {
        ServiceTransportBinding::ProviderHttpJson => RemoteModuleTransport::HttpJson,
        ServiceTransportBinding::ProviderGrpc => RemoteModuleTransport::Grpc,
        _ => unreachable!("endpoint selection accepts only Provider transports"),
    };
    let config = RemoteModuleConfig::new(&locked.module_id, &endpoint.address)
        .with_transport(transport, &endpoint.address)
        .for_service_module(&locked.module_id);
    match bearer {
