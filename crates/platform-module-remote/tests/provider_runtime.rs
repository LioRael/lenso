...
async fn spawn_descriptor(manifest: ModuleManifest, sibling: Option<ModuleManifest>) -> String {
    ...
    let app = Router::new().route(
        "/modules/acme/support/manifest",
        get(move || {
            let manifest = manifest.clone();
            let sibling = sibling.clone();
            async move {
                let mut modules = vec![manifest];
                modules.extend(sibling);
                Json(json!({
                    "name": "acme/support-service",
                    "version": "1.0.0",
                    "modules": modules,
                }))
            }
...
async fn rejects_live_descriptor_that_differs_from_locked_manifest() {
    ...
    assert!(error.public_message.contains("locked Manifest"));
}
